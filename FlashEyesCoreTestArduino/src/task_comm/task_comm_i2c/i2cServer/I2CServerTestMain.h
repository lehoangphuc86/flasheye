...
#ifndef _I2C_SERVER_TEST_MAIN_H
#define _I2C_SERVER_TEST_MAIN_H
...
/////////////////////////////////////////////////
// PREPROCESSOR
#define I2C_SERVER_TEST_DEBUG_CONSOLE_ENABLED
/////////////////////////////////////////////////
// DEFINE
...
protected:
#if (1)
  static I2CServerTask comm_I2C_Test;
#endif //0
};

I2CServerTask I2CServerTest::comm_I2C_Test;

I2CServerTest::I2CServerTest()
{
...
int I2CServerTest::setup()
{
  ...
  do
  {
...
#if (TEST_THREAD_POOL_COUNT)
    {
      ...
      threadPoolConfig[0].dataItemConfig.usStackDepth = I2C_COMM_TEST_TASK_MEM;
      threadPoolConfig[0].dataItemConfig.uxPriority = I2C_COMM_TEST_TASK_PRIORITY;

      memcpy(&threadPoolConfig[1], &threadPoolConfig[0], sizeof(TaskThreadManagerConfigTAG));
      threadPoolConfig[1].preparedDataNumber = 1;
      threadPoolConfig[1].dataItemConfig.usStackDepth = I2C_COMM_TEST_TASK_MEM + 10;

      ret = TaskThreadPoolManager::getInstance().prepare(TEST_THREAD_POOL_COUNT, threadPoolConfig);
      if (ret != 0)
      {
        break;
      }
...
#if (TEST_BUFFER_POOL_COUNT)
    {
      ...
      bufferPoolConfig[0].preparedDataNumber = I2C_COMM_TEST_SERVER_BUFFER_COUNT;
...
#if (TEST_EVENT_POOL_COUNT)
    {
      ...
      eventPoolConfig[0].preparedDataNumber = I2C_COMM_TEST_TASK_EVENT_NO;
      ...
      eventPoolConfig[1].preparedDataNumber = I2C_COMM_TEST_TASK_EVENT_NO;
      eventPoolConfig[1].dataItemConfig.eventItemSize = sizeof(EventCommI2CServerDataRequestedTAG);

      ret = EventPoolManager::getInstance().prepare(TEST_EVENT_POOL_COUNT, eventPoolConfig);
      if (ret != 0)
      {
        break;
      }
    }
#endif // (TEST_EVENT_POOL_COUNT)

#if defined(I2C_COMM_TEST_SERVER_USE_TASK) || defined(OPCODE_INPUT_TAS_USE_TASK)
    // Set up tasks
    ...
    // set up server
    {
      ...
      commConfig.bufferCount = I2C_COMM_TEST_SERVER_BUFFER_COUNT;
      ...
      commConfig.maxClientNumber = I2C_COMM_TEST_SERVER_MAX_CLIENT_COUNT;
      commConfig.codecType = TaskCodecConstant::TaskCodecType::TASK_CODEC_HEX;

      commConfig.isHeaderless(I2C_COMM_TEST_SERVER_CONNECTION_HEADERLESS); // not used
      commConfig.parentPointer = NULL; // not used
      commConfig.cbDataReceived = NULL; // not used
      commConfig.cbDataRequested = NULL; // not used
      
#if defined(I2C_COMM_TEST_SERVER_USE_TASK)
      taskManagerConfig.eventItemNumber = I2C_COMM_TEST_TASK_EVENT_NO;
      
      ret = I2CServerTest::comm_I2C_Test.setConfig(commConfig, taskManagerConfig);
#else // I2C_COMM_TEST_SERVER_USE_TASK
      ret = I2CServerTest::comm_I2C_Test.setConfig(commConfig);
...

    {
      ...
      ret = OpcodeInputTask::getInstance().setConfig(
        &menuItem
        , 1
        , I2CServerTest::cbOpCodeReceived
#ifdef OPCODE_INPUT_TAS_USE_TASK
        , taskManagerConfig
#endif // OPCODE_INPUT_TAS_USE_TASK
      );
...

    {
      ...
      commI2CSocketConfig.busNumer = I2C_COMM_TEST_SERVER_SOCKET_BUS_NUMBER;
      commI2CSocketConfig.pin_SCL = I2C_COMM_TEST_SERVER_SOCKET_PIN_SCL_0;
      commI2CSocketConfig.pin_SDA = I2C_COMM_TEST_SERVER_SOCKET_PIN_SDA_0;
      commI2CSocketConfig.frequency = I2C_COMM_TEST_SERVER_SOCKET_FREQUENCY;
      commI2CSocketConfig.address = I2C_COMM_TEST_SERVER_SOCKET_ADDRESS;
      commI2CSocketConfig.timeout = I2C_COMM_TEST_SERVER_SOCKET_TIMEOUT;
#ifdef I2C_COMM_TEST_SERVER_USE_TASK
      TaskThreadConfigTAG threadConfig = TaskThreadConfigTAG();
      threadConfig.enabled = true;
      threadConfig.usStackDepth = I2C_COMM_TEST_TASK_MEM;
      threadConfig.uxPriority = I2C_COMM_TEST_TASK_PRIORITY;
      threadConfig.useThreadPool = TaskThreadPoolManager::getInstance().isValid();
      ret = I2CServerTest::comm_I2C_Test.start(commI2CSocketConfig, threadConfig);
#else // I2C_COMM_TEST_SERVER_USE_TASK
    ret = I2CServerTest::comm_I2C_Test.start(commI2CSocketConfig);
#endif // I2C_COMM_TEST_SERVER_USE_TASK
      if (ret != 0)
      {
        break;
...
bool I2CServerTest::cbOpCodeReceived(int opCode)
{
#ifdef I2C_SERVER_TEST_DEBUG_CONSOLE_ENABLED
  CONSOLE_LOG("[seT] oprev %i", opCode);
#endif // I2C_SERVER_TEST_DEBUG_CONSOLE_ENABLED
  
  bool isStopped = false;
  int ret = 0;
  static byte messageId = 124;
  ...
  {
    //clientIndexSend++;
    clientAddress = (clientIndexSend%2 == 0? I2C_COMM_TEST_SERVER_CLIENT_SOCKET_1_ADDRESS : I2C_COMM_TEST_SERVER_CLIENT_SOCKET_2_ADDRESS);
  }

  if (opCode == OP_REQUEST)
  {
    //clientIndexRequest++;
    clientAddress = (clientIndexRequest % 2 == 0 ? I2C_COMM_TEST_SERVER_CLIENT_SOCKET_1_ADDRESS : I2C_COMM_TEST_SERVER_CLIENT_SOCKET_2_ADDRESS);
  }
  ...
  do
  {
    ...
    {
    case OP_SEND:
      ret = I2CServerTest::comm_I2C_Test.send(clientAddress, ++messageId);
      break;
    case OP_REQUEST:
      ret = I2CServerTest::comm_I2C_Test.request(clientAddress, ++messageId, sizeof(CommI2CTestHeadlessDataTAG));
      break;
    default:
      break;
    }
